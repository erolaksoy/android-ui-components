...
internal object TimelineViewItemProvider {
    ...
    private fun getImageConfig(
        ...
        return ImageConfig(
            ...
            placeholder = null,
            animation = if (animationEnabled) PointAnimation() else null
        )
    }
    ...
    private fun getTextStyle(color: Color): TextStyle {
        return TextStyle(
            color = color,
    ...
    internal fun createTimelineImageItems(): List<TimelineItem.Image> {
        //active
        val item1 = TimelineItem.Image(
            ...
            lineConfig = getLineConfig(
                colorGray,
                lineAnimation = LineAnimation(targetColor = colorGreen)
            ),
        )
        ...
        val item3 = TimelineItem.Image(
            imageConfig = getImageConfig(isActive = true, animationEnabled = true),
            text = "İade İnceleniyor",
            textStyle = getTextStyle(colorGreen),
            lineConfig = getLineConfig(colorGray),
        )
